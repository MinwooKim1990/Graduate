...
The part which needs to be modified, as compared to the lecture, is the `data` argument of the `jags.model` call; everything else stays the same. At the end of the process, also produce the `summary' output of the fitted model. Compare the parameter estimates and standard errors with those obtained previously.
 
```{r}
require(rjags)
model1 <- jags.model(textConnection(model1_string), 
    data = list(x=realestate$X3.MRT, y=realestate$Y,N=length(realestate$Y))
    )

update(model1, 10000)

postmod1.samples = coda.samples(model1, c("beta0", "beta1", "sigma"), 10000)[[1]]
dim(postmod1.samples)
...
postmod3.samples = coda.samples(model3, c("beta0", "beta1", "tau"), 10000)[[1]]
```
...
rbind(rowMeans(t(postmod3.samples[,1:2])),
      real.fit.l1$coef[1:2])

rbind(round(apply(t(postmod3.samples[,1:2]), 1,sd), digits=4),
